...
def random_forest(train, test, max_depth, min_size, sample_size, n_trees, n_features):
    ...
    Args:
        ...
        n_features      选取的特征的个数
    Returns:
        predictions     每一行的预测结果，bagging 预测最后的分类结果,输出最后结果
    """
...
def evaluate_algorithm(dataset, algorithm, n_folds, *args):
    ...
    for fold in folds:
        ...
        In[27]: sum([[1,2,3,'a'],[4,5,6,'b']],[])
        Out[27]: [1,2,3,4,5,6,'a','b']
        In[28]: sum([[[1,2,3,'a'],[4,5,6,'b']]],[])
        Out[28]: [[1, 2, 3, 'a'], [4, 5, 6, 'b']]
        """
        train_set = sum(train_set, [])#最外层的中括号没有了，2个外层中括号变一个
        test_set = list()
        ...
        predicted = algorithm(train_set, test_set, *args) #这个是random_forest方法,predicted 这个是预测的结果
...
if __name__ == '__main__':
    # 加载数据
    dataset = loadDataSet(traindatadir+'input/7.RandomForest/sonar-all-data.txt')
    # print dataset
