...
def spamTest():
    ...
    for i in range(1, 26):
        # 切分，解析数据，并归类为 1 类别
        wordList = textParse(open(traindatadir+'input/4.NaiveBayes/email/spam/%d.txt' % i).read())
        docList.append(wordList)
        classList.append(1)
        # 切分，解析数据，并归类为 0 类别
        wordList = textParse(open(traindatadir+'input/4.NaiveBayes/email/ham/%d.txt' % i,encoding='gbk').read())
        docList.append(wordList)
        fullText.extend(wordList)
        classList.append(0)
    # 创建词汇表
    vocabList = createVocabList(docList)
    trainingSet = list(range(50))#需要一个列表，不然报错，python2并不会报错
    testSet = []
    # 随机取 10 个邮件用来测试
    for i in range(10):
...
#文档词袋模型
#我们将每个词的出现与否作为一个特征，这可以被描述为 词集模型(set-of-words model)。如果一个词在文档中出现不止一次，这可能意味着包含该词是否出现在文档中所不能表达的某种信息，
# 这种方法被称为 词袋模型(bag-of-words model)。在词袋中，每个单词可以出现多次，而在词集中，每个词只能出现一次。为适应词袋模型，需要对函数 setOfWords2Vec() 稍加修改，修改后的函数为 bagOfWords2Vec() 。
# 将文本文件解析成 词条向量
...
def bagOfWords2VecMN(vocaList, inputSet):
    ...
    for word in inputSet:
        if word in vocaList:
            returnVec[vocaList.index(word)] += 1#这里是+1
        else:
            print("the word: %s is not in my Vocabulary!" % word)
    return returnVec
...
def localWords(feed1,feed0):
    ...
    trainingSet=list(range(2*minLen));testSet=[]##不加list 报错：TypeError: 'range' object doesn't support item deletion，原因是python3中range不返回数组对象，而是返回range对象
    for i in range(20):
        randIndex=int(random.uniform(0,len(trainingSet)))
        testSet.append(trainingSet[randIndex])
        del(trainingSet[randIndex])
