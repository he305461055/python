...
# sigmoid跳跃函数 #这玩意看看介绍，原文有
def sigmoid(inX):
     return 1.0 / (1 + exp(-inX)) #二值型输出分类函数
...
def testLR():
    ...
    # 1.收集并准备数据
    dataMat, labelMat = loadDataSet(traindatadir+"input/5.Logistic/TestSet.txt")
...
def colicTest():
    ...
    Returns:
        errorRate -- 分类错误率
    '''
    frTrain = open(traindatadir+'input/5.Logistic/horseColicTraining.txt')
    frTest = open(traindatadir+'input/5.Logistic/horseColicTest.txt')
    trainingSet = []
    trainingLabels = []
